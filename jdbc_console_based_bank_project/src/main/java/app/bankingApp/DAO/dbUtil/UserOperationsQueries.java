...
//			+ "VALUES(?,?,?,?,?,?,?,?,(select status_id from project_console_bank.user_status where status_user=?))";
	...
	public static final String GETUSERBYID = "SELECT user_id, first_name, last_name, dob, phone_number, email, "
			...
			+ " LEFT JOIN project_console_bank.non_customer_user_for_approval as us_to_c ON u.user_id = us_to_c.user_id_cust_appr "
			+ " WHERE user_id = ? ORDER BY last_name ASC";
